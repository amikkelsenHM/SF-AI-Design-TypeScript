...
import { PIN_CONFIG } from '../map-chart/constants/map-constants';

const DEFAULT_SIZE = PIN_CONFIG.SIZE;
const SELECTED_COLOR = PIN_CONFIG.COLORS.SELECTED;
const DEFAULT_COLOR = PIN_CONFIG.COLORS.DEFAULT;

const getFillColor = (props: LocationPinProps): string => {
  const { fill, color, isSelected } = props;
...
export const LocationPinIcon: React.FC<LocationPinProps> = ({
  size = DEFAULT_SIZE,
  className = '',
  'aria-label': ariaLabel = 'Location pin',
  ...props
}) => {
  const { color, isSelected, fill, ...svgProps } = props;
